                ...
                <v-card-text>
                  <v-form ref="form">
                    <v-container>
                      <v-row>
                          <v-col>
                              <v-select v-model="editedItem.group_type" :rules="subGroup1Rules1" :items="group_type" label="Group Type">
                              </v-select>
                            </v-col>
                            <v-col>
                              <v-select v-model="editedItem.group_name" :rules="subGroup1Rules2" :items="group_name" label="Group name">
                              </v-select>
                            </v-col>
                       </v-row>
                      <v-row>
                          <v-col>
                              <v-select v-model="editedItem.sub_group_name" :rules="subGroup1Rules3" :items="sub_group_name" label="Sub Group name">
                              </v-select>
                            </v-col>
                            <v-col>
                              <v-text-field
                                  v-model="editedItem.sub_group_name"
                                  :rules="subGroup1Rules5"
                         ...
                         <v-row>
                            <v-col v-if="status_val">
                              <v-select v-model="editedItem.status" :rules="subGroup1Rules6" :items="status" label="Status">
                              </v-select>
                            </v-col>
                            <v-col>
                              <v-text-field
...
export default {
  data: () => ({
   status:['Yes','No'], 
    ...
    items: [
      ...
      {
        text: "Sub Group1 Master",
        disabled: false,
        href: "subGroup1Master"
      }
    ],
    /*breadcrumb*/
    dialog: false,
    ...
    subGroup1Rules1: [
      ...
      v => !!v || "Sub Group id1 is required",
      v => (v && v.length >= 3) || "Sub Group id1 is required"
    ],subGroup1Rules5: [
      v => !!v || "Sub Group Name is required",
      v => (v && v.length >= 3) || "Sub Group Name is required"
    ],subGroup1Rules6: [
    ...
    headers: [
      ...
      {
        text: "Sub Group1 Name",
        align: "center",
        sortable: false,
        value: "sub_group1_name",
        width: 80
      },
      {
        text: "Status",
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "Sub Group1 Master" : "Edit Group";
    }
  },
